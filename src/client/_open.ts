...
import type { DialOptions } from "./types.ts";

const clientTracer = trace.getTracer('ddp.client');

export async function openWebsocketStream(props: DialOptions): Promise<WebSocketConnection> {
  let sockPath = 'websocket';

  if (props.encapsulation == 'sockjs') {
  ...
  const sockUrl = new URL(sockPath, props.appUrl);
